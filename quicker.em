...
macro DoxygenNote()
{
    ...
    if(sel.lnFirst == sel.lnLast && sel.ichFirst == sel.ichLim)
    {
        ...
        newLine=strmid(szLine, 0, sel.ichFirst)r
        newLine=cat(newLine, "///@note")r
        if(hasTail==1)
        {
...
function FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if(newFunc != 1)
    {
        ...
        while ( i < lnMax) 
        {
            ...
            InsBufLine(hbuf, ln+2, "@szTmp@")   //在第ln+2行插入是因为前面每个参数都ln=ln+1，其实是从ln3行开始每个参数插入一行
            iIns = 1    //函数是否存在参数标记
            szIns = " *             "       //第二个参数前面留空格对齐
            i = i + 1
        }    
    ...
    {       
            ln = ln + 1     //为了保持如有参数的一致
            InsBufLine(hbuf, ln+2, " * 输入参数  : 无")  //在第ln+2行插入是因为前面每个参数都ln=ln+1，其实是从ln3行开始每个参数插入一行
    }
    ...
    sz2=SysTime.month
    sz3=SysTime.day
    InsBufLine(hbuf, ln+6, " * 创建日期  : @sz1@年@sz2@月@sz3@日")
    ...
    //显示输入的功能描述内容
    newln = CommentContent(hbuf,oldln+2," * 函数功能  : ",szContent,0) - 2//此处减4是因为CommentContent返回的值是传进去的oldln+4之后演算出来的
...
/*****************************************************************************
 ...
 修改历史      :
  ...
  2.日    期   : 2026年1月18日
    作    者   : 0Litost0
    修改内容   : 使用0Litost0修改的doxygen格式的注释头
